#!/usr/bin/env python3
"""
Validación del clasificador Woz (LLM local vía Ollama) contra eval_v1.jsonl.
...
from woz_client import WozPrediction, classify_with_woz  # noqa: E402
...
def run_eval(
    ...
    for r in rows:
        ...
        lines_out.append(f"{mark}  {eid}  expected={exp}  got={pred.intent}  conf={pred.confidence:.3f}")
        if not ok:
...
def run_interactive(record_chat: bool, errors_path: Path) -> None:
    print("Woz — escribe frases para clasificar (vacío o Ctrl-D para salir).")
    if record_chat:
    ...
    while True:
        ...
        if not line:
            break
        pred = classify_with_woz(line)
        print(f"  intent: {pred.intent}  confidence: {pred.confidence:.3f}")
        if record_chat:
            try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Valida Woz (LLM local) contra eval_v1.jsonl")
    parser.add_argument("--repo-root", type=Path, default=None)
    parser.add_argument("--eval", type=Path, default=None)
    parser.add_argument("-m", "--message", default=None, help="Una frase; imprime intención")
    ...
    parser.add_argument("--record-chat", action="store_true")
    ...
    if args.message is not None:
        pred = classify_with_woz(args.message)
        print(pred.intent)
        return

    if args.interactive:
        run_interactive(args.record_chat, errors_path)
        return
